...
const Radar = function (size, radar) {
  ...
  function plotTexts(erase) {
    var svg = d3.select('#radar-plot');
    var texts = svg.selectAll('.line-text');
    if (erase) {
      if (! texts.empty()) {
        texts.remove();
      }
    } else if (texts.empty()){
      ['adopt', 'trial', 'assess', 'hold'].forEach(function (name, i) {
        svg.append('text')
    ...
    rings.forEach(function (ring, i) {
      ...
      ringBlips.forEach(function (blip) {
        ...
        if (iterationCounter < maxIterations) {
          ...
          var textDiv = blipListItem.append('div')
            .attr('class', 'blip-list-item')
            .text(blipText);

          if(blip.isNew()){
            textDiv.append("span")
              .attr("class", "blip-new-indicator")
              .text("new");
          }
  ...
  function removeHomeLink() {
    d3.select('.home-link').style('visibility', 'hidden');
  }
